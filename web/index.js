...
itemsArray_to_send_to_courier=[
  ...itemsArray
          ...
          const payload = {
            ...
            request_type: "wp",
            pickupFirstName: items[0].pickupLocation?.first_name?? "NA",
            pickupLastName: items[0].pickupLocation?.last_name?? "NA",
            pickupCompanyName: CompanyName ?? "NA",
            pickupEmail: items[0].pickupLocation?.email?? "NA",
            pickupAddress1: items[0].pickupLocation?.address1 ?? "NA",
            pickupAddress2: items[0].pickupLocation?.address2 ?? "NA",
            pickupPhone: items[0].pickupLocation?.phone?? "NA",
            pickupSuburb: items[0].pickupLocation?.suburb?? "NA",
            pickupState: items[0].pickupLocation?.state?? "NA",
            pickupPostcode: items[0].pickupLocation?.postcode?? "NA",
            pickupBuildingType: items[0].pickupLocation?.building_type?? "NA",
            pickupTimeWindow: `${items[0].pickupLocation?.time_window?? "NA"}`,
            isPickupTailLift: `${
              merchant?.is_drop_off_tail_lift
                ? Number(totalWeightOfItems > merchant.weighht)
                : 0
            }`,
            destinationSuburb: destination.city ?? "NA",
            destinationState: destination.province ?? "NA",
            destinationPostcode: destination.postal_code ?? "NA",
            destinationBuildingType: destination.company
              ? "commercial"
              : "residential",
            destinationFirstName: destination.name,
            destinationLastName: "NA",
            destinationCompanyName: "NA",
            destinationEmail: destination.email ?? "NA",
            destinationAddress1: destination.address1?? "NA",
            destinationAddress2: destination.address2 ?? "NA",
            destinationPhone: destination.phone?? "NA",
