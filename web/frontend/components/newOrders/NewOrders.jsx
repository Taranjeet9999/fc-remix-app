      ...
      if (orderDataEdge) {
        ...
        for (let index = 0; index < _orderArray.length; index++) {
          ...
          _orderArray[index] = {
            ...
            wpOrderId:element?.id  ?? "NA",
            collectionDate: collectionDate  ?? "NA",
            destinationEmail: element?.contact_email  ?? "NA",
            destinationPhone: element?.shipping_address?.phone  ?? "NA" , 
            destinationFirstName: element?.shipping_address?.first_name ?? "NA",
            destinationLastName: element?.shipping_address?.last_name ?? "NA",
            destinationCompanyName: element?.shipping_address.company ?? "NA",
            destinationAddress1: element?.shipping_address.address1 ?? "NA",
            destinationAddress2: element?.shipping_address.address2 ?? "NA",
            pickupFirstName: defaultLocation?.first_name ?? "NA",
            pickupLastName: defaultLocation?.last_name ?? "NA",
            pickupCompanyName:   "NA",
            pickupAddress1: defaultLocation?.address1 ?? "NA",
            pickupAddress2: defaultLocation?.address2 ?? "",
            pickupPhone: defaultLocation?.phone ?? "NA",
            pickupEmail: defaultLocation?.email ?? "NA",
            atl: false, 
            // ...order,
          };
        }
